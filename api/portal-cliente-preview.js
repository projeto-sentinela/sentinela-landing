// Preview do portal vitalicio do CLIENTE para WhatsApp/redes.
// Recebe /pc/{token} (via rewrite do vercel.json), busca capa/foto/nomes daquele
// portal no Supabase e devolve as tags Open Graph com a imagem e textos certos.
...
const APP_PORTAL_BASE = "https://app.sentinelapro.com/c/";
const FALLBACK_IMAGE = "https://sentinelapro.com/og-portal.png";
const BUCKET = "portal-media";
const SITE_NAME = "Sentinela";
const FALLBACK_TITLE = "Sua história";
const FALLBACK_DESCRIPTION = "Sentinela";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (token && SUPABASE_URL && SUPABASE_ANON_KEY) {
      ...
      const r = await fetch(`${SUPABASE_URL}/rest/v1/rpc/get_portal_cliente_publico`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({ _token: token, _senha: null }),
      ...
      if (r.ok) {
        ...
        if (row && row.publicado !== false) {
          if (row.cliente_nome) title = `A história de ${row.cliente_nome}`;
          if (row.fotografa_nome)
            description = `Acompanhada por ${row.fotografa_nome} · Sentinela`;
          const rawPath = row.capa_url || row.cliente_foto_url || null;
    ...
    // qualquer erro -> usa os fallbacks genericos (marca)
  }
...
<meta property="og:url" content="https://sentinelapro.com/pc/${encodeURIComponent(token)}">
...
// Le o token de qualquer forma que a Vercel entregar (query, URL crua ou path /pc/xxx)
